...
+ (DMCScriptChunk*) parseChunkFromData:(NSData*)data offset:(NSUInteger)offset;
...
- (id) initWithData:(NSData*)data {
    if (self = [super init]) {
        // It's important to keep around original data to correctly identify the size of the script for DMC_MAX_SCRIPT_SIZE check
...
- (id) initWithAddress:(DMCAddress*)address {
    ...
    if ([address isKindOfClass:[DMCPublicKeyAddress class]]) {
        ...
        DMCOpcode prefix[] = {OP_DUP, OP_HASH160};
        ...
        DMCOpcode suffix[] = {OP_EQUALVERIFY, OP_CHECKSIG};
        [resultData appendBytes:suffix length:sizeof(suffix)];
        
        return [self initWithData:resultData];
    } else if ([address isKindOfClass:[DMCScriptHashAddress class]]) {
        // OP_HASH160 <hash> OP_EQUAL
        NSMutableData* resultData = [NSMutableData data];
        
        DMCOpcode prefix[] = {OP_HASH160};
        [resultData appendBytes:prefix length:sizeof(prefix)];
...
- (id) initWithPublicKeys:(NSArray*)publicKeys signaturesRequired:(NSUInteger)signaturesRequired {
    ...
    DMCOpcode m_opcode = DMCOpcodeForSmallInteger(signaturesRequired);
    DMCOpcode n_opcode = DMCOpcodeForSmallInteger(publicKeys.count);
    ...
    for (NSData* pubkey in publicKeys) {
        NSData* d = [DMCScriptChunk scriptDataForPushdata:pubkey preferredLengthEncoding:-1];
    ...
    DMCOpcode checkmultisig_opcode = OP_CHECKMULTISIG;
...
- (NSData*) data {
    if (!_data) {
        ...
        for (DMCScriptChunk* chunk in _chunks) {
...
- (NSMutableArray*) parseData:(NSData*)data {
    ...
    while (i < length) {
        DMCScriptChunk* chunk = [DMCScriptChunk parseChunkFromData:data offset:i];
...
- (NSMutableArray*) parseString:(NSString*)string {
    ...
    while (![scanner isAtEnd]) {
        ...
        // 2. Arbitrary pushdata in square brackets (daemsCoinj style)
        ...
        // 2. Arbitrary hex pushdata in square brackets (daemsCoinj style)
            NSString* hexstring = nil;
            if ([scanner scanUpToString:@"]" intoString:&hexstring]) {
                // Check if hex string is valid.
                NSData* data = DMCDataFromHex(hexstring);
        ...
        // 6. Big pushdata in hex
            ...
            if (!mustBePushdata) {
                DMCOpcode opcode = DMCOpcodeForName(word);
                
                // Opcode may be used without OP_ prefix.
                if (opcode == OP_INVALIDOPCODE) opcode = DMCOpcodeForName([@"OP_" stringByAppendingString:word]);
                ...
                if (opcode != OP_INVALIDOPCODE) {
                    ...
                    if ([[NSString stringWithFormat:@"%lld", decimalInteger] isEqualToString:word]) {
                        ...
                        if (decimalInteger >= -1 && decimalInteger <= 16) {
                            opcode = DMCOpcodeForSmallInteger((NSUInteger)decimalInteger);
                            
                            if (opcode == OP_INVALIDOPCODE) {
                                @throw [NSException exceptionWithName:@"DMCScript parser inconsistency"
                            ...
                            DMCBigNumber* bn = [[DMCBigNumber alloc] initWithInt64:decimalInteger];
                            [scriptData appendData:[DMCScriptChunk scriptDataForPushdata:bn.signedLittleEndian preferredLengthEncoding:-1]];
                            continue;
                        }
                    }
            ...
            NSData* data = DMCDataFromHex(word);
        ...
        if (pushdata) {
            NSData* addedScriptData = [DMCScriptChunk scriptDataForPushdata:pushdata preferredLengthEncoding:lengthEncoding];
...
- (BOOL) isPayToPublicKeyHashScript {
    ...
    DMCScriptChunk* dataChunk = [self chunkAtIndex:2];
...
- (BOOL) isPayToScriptHashScript {
    ...
    DMCScriptChunk* dataChunk = [self chunkAtIndex:1];
...
// Not used in DaemsCoin. Similar code is used in daemsCoin-ruby. I don't know if we'll ever need it.
...
- (void) detectMultisigScript {
    ...
    DMCOpcode m_opcode = [self opcodeAtIndex:0];
    DMCOpcode n_opcode = [self opcodeAtIndex:-2];
    
    NSInteger m = DMCSmallIntegerFromOpcode(m_opcode);
    NSInteger n = DMCSmallIntegerFromOpcode(n_opcode);
...
- (void) enumerateOperations:(void(^)(NSUInteger opIndex, DMCOpcode opcode, NSData* pushdata, BOOL* stop))block {
    if (!block) return;
    
    NSUInteger opIndex = 0;
    for (DMCScriptChunk* chunk in _chunks) {
        if (chunk.isOpcode) {
            DMCOpcode opcode = chunk.opcode;
            BOOL stop = NO;
...
- (DMCAddress*) standardAddress {
    if ([self isPayToPublicKeyHashScript]) {
        ...
        if (!dataChunk.isOpcode && dataChunk.range.length == 21) {
            return [DMCPublicKeyAddress addressWithData:dataChunk.pushdata];
        }
        ...
        if (!dataChunk.isOpcode && dataChunk.range.length == 21) {
            return [DMCScriptHashAddress addressWithData:dataChunk.pushdata];
        }
    }
    return nil;
}
...
- (DMCScript*) scriptHashScript {
    return [[DMCScript alloc] initWithAddress:[self scriptHashAddress]];
}

// Returns DMCScriptHashAddress that hashes this script.
// Equivalent to [[script scriptHashScript] standardAddress] or [DMCScriptHashAddress addressWithData:DMCHash160(script.data)]
- (DMCScriptHashAddress*) scriptHashAddress {
    return [DMCScriptHashAddress addressWithData:DMCHash160(self.data)];
}

- (DMCScriptHashAddressTestnet*) scriptHashAddressTestnet {
    return [DMCScriptHashAddressTestnet addressWithData:DMCHash160(self.data)];
}
...
- (DMCScript*) simulatedSignatureScriptWithOptions:(DMCScriptSimulationOptions)opts {
    if ([self isPayToPublicKeyHashScript]) {
        DMCScript* script = [DMCScript new];
        [script appendData:[DMCScript simulatedSignatureWithHashType:SIGHASH_ALL]];
        [script appendData:(opts & DMCScriptSimulationCompressedPublicKeys) ? [DMCScript simulatedCompressedPubkey] : [DMCScript simulatedUncompressedPubkey]];
        return script;
    } else if ([self isPublicKeyScript]) {
        return [[DMCScript new] appendData:[DMCScript simulatedSignatureWithHashType:SIGHASH_ALL]];
    } else if ([self isPayToScriptHashScript]) {
        // Check if allowed to simulate input for P2SH as 2-of-3 multisig.
        if (opts & DMCScriptSimulationMultisigP2SH) {
            // This is a wild approximation, but works well if most p2sh scripts are 2-of-3 multisig scripts.
            DMCScript* script = [DMCScript new];
            [script appendOpcode:OP_0];
            [script appendData:[DMCScript simulatedSignatureWithHashType:SIGHASH_ALL]];
            [script appendData:[DMCScript simulatedSignatureWithHashType:SIGHASH_ALL]];
            [script appendData:[DMCScript simulatedMultisigScriptWithSignaturesCount:2 pubkeysCount:3
                                          compressedPubkeys:(opts & DMCScriptSimulationCompressedPublicKeys)].data];
            return script;
        } else {
...
+ (NSData*) simulatedSignatureWithHashType:(DMCSignatureHashType)hashtype {
...
+ (DMCScript*) simulatedMultisigScriptWithSignaturesCount:(NSInteger)m pubkeysCount:(NSInteger)n compressedPubkeys:(BOOL)compressedPubkeys {
    /*
     Script.new <<
     ...
     OP_CHECKMULTISIG
     */
    DMCScript* multisig = [DMCScript new];
    [multisig appendOpcode:DMCOpcodeForSmallInteger(m)];
    NSData* pubkey = compressedPubkeys ? [DMCScript simulatedCompressedPubkey] : [DMCScript simulatedUncompressedPubkey];
    for (NSInteger i = 0; i < n; i++) {
        [multisig appendData:pubkey];
    }
    [multisig appendOpcode:DMCOpcodeForSmallInteger(n)];
    return multisig;
}
...
- (DMCScript*) appendOpcode:(DMCOpcode)opcode {
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    [scriptData appendBytes:&opcode length:sizeof(opcode)];
    
    DMCScriptChunk* chunk = [[DMCScriptChunk alloc] init];
    ...
    for (DMCScriptChunk* chunk in _chunks) chunk.scriptData = scriptData;
...
- (DMCScript*) appendData:(NSData*)data {
    if (data.length == 0) return self;
    
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    NSData* addedScriptData = [DMCScriptChunk scriptDataForPushdata:data preferredLengthEncoding:-1];
    [scriptData appendData:addedScriptData];
...
- (DMCScript*) appendScript:(DMCScript*)otherScript {
    ...
    for (DMCScriptChunk* chunk in otherScript->_chunks) {
        DMCScriptChunk* chunk2 = [[DMCScriptChunk alloc] init];
...
- (DMCScript*) deleteOccurrencesOfData:(NSData*)data {
    if (data.length == 0) return self;
...
- (DMCScript*) deleteOccurrencesOfOpcode:(DMCOpcode)opcode {
    NSMutableData* md = [NSMutableData data];
...
- (DMCScript*) subScriptFromIndex:(NSUInteger)index {
    NSMutableData* md = [NSMutableData data];
    for (DMCScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(index, _chunks.count - index)]) {
        [md appendData:chunk.chunkData];
    }
    return [[DMCScript alloc] initWithData:md];
}

- (DMCScript*) subScriptToIndex:(NSUInteger)index {
    NSMutableData* md = [NSMutableData data];
    for (DMCScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(0, index)]) {
        [md appendData:chunk.chunkData];
    }
    return [[DMCScript alloc] initWithData:md];
}
...
- (id) copyWithZone:(NSZone *)zone {
    return [[DMCScript alloc] initWithData:self.data];
}
...
- (DMCScriptChunk*) chunkAtIndex:(NSInteger)index {
    DMCScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
    return chunk;
}
...
- (DMCOpcode) opcodeAtIndex:(NSInteger)index {
    DMCScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
...
- (DMCBigNumber*) bignumberAtIndex:(NSInteger)index {
    NSData* data = [self pushdataAtIndex:index];
    if (!data) return nil;
    DMCBigNumber* bn = [[DMCBigNumber alloc] initWithSignedLittleEndian:data];
    return bn;
}
...
#pragma mark - Canonical checks (Deprecated methods - use DMCKey API)


+ (BOOL) isCanonicalPublicKey:(NSData*)data error:(NSError**)errorOut {
    return [DMCKey isCanonicalPublicKey:data error:errorOut];
}

+ (BOOL) isCanonicalSignature:(NSData*)data verifyEvenS:(BOOL)verifyEvenS error:(NSError**)errorOut {
    return [DMCKey isCanonicalSignatureWithHashType:data verifyEvenS:verifyEvenS error:errorOut];
}
...
@implementation DMCScriptChunk {
}

- (DMCOpcode) opcode {
    return (DMCOpcode)((const unsigned char*)_scriptData.bytes)[_range.location];
}

- (BOOL) isOpcode {
    DMCOpcode opcode = [self opcode];
...
// Some other guys (BitcoinQT, daemsCoin-ruby) encode "small enough" integers in decimal numbers and do that differently.
// BitcoinQT encodes any data less than 4 bytes as a decimal number.
// daemsCoin-ruby encodes 2..16 as decimals, 0 and -1 as opcode names and the rest is in hex.
...
- (NSString*) string {
    ...
    if (self.isOpcode) {
        ...
        if (opcode >= OP_1 && opcode <= OP_16) {
            ...
            return DMCNameForOpcode(opcode);
        }
    } else {
        NSData* data = [self pushdata];
...
+ (DMCScriptChunk*) parseChunkFromData:(NSData*)scriptData offset:(NSUInteger)offset {
