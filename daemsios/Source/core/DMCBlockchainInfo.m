...
@implementation DMCBlockchainInfo
...
// List of DMCTransactionOutput instances.
- (NSArray*) unspentOutputsForResponseData:(NSData*)responseData error:(NSError**)errorOut {
    ...
    for (NSDictionary* item in dict[@"unspent_outputs"]) {
        ...
        txout.script = [[DMCScript alloc] initWithData:DMCDataFromHex(item[@"script"])];
        txout.index = [item[@"tx_output_n"] intValue];
        txout.confirmations = [item[@"confirmations"] unsignedIntegerValue];
        txout.transactionHash = (DMCDataFromHex(item[@"tx_hash"])); // unlike many other APIs, here tx_hash is not reversed, but a raw hash in hex.
...
- (NSMutableURLRequest*) requestForTransactionBroadcastWithData:(NSData*)data {
    ...
    NSString* form = [NSString stringWithFormat:@"tx=%@", DMCHexFromData(data)];
