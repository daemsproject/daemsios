...
typedef NS_ENUM(NSInteger, DMCScriptSimulationOptions) {
    // Default is strict conservative simulation (uncompressed pubkeys, no P2SH interpretation)
    DMCScriptSimulationDefault = 0,

    // Use compressed pubkeys instead of uncompressed ones.
    DMCScriptSimulationCompressedPublicKeys = 1 << 0,

    // Instead of failing on P2SH, assume it is 2-of-3 multisig (the most popular one).
    DMCScriptSimulationMultisigP2SH = 1 << 1,
};

@class DMCAddress;
@class DMCScriptHashAddress;
@class DMCScriptHashAddressTestnet;
@interface DMCScriptChunk : NSObject
...
@property(nonatomic, readonly) DMCOpcode opcode;
...
@interface DMCScript : NSObject<NSCopying>
...
// List of parsed chunks of the script (DMCScriptChunk)
...
// This is the most popular type that is used to pay to "addresses" (e.g. 1CDMCGivXmHQ8ZqdPgeMfcpQNJrqTrSAcG).
...
// Returns DMCPublicKeyAddress or DMCScriptHashAddress if the script is a standard output script for these addresses.
// If the script is something different, returns nil.
@property(nonatomic, readonly) DMCAddress* standardAddress;

// Wraps the recipient into an output P2SH script (OP_HASH160 <20-byte hash of the recipient> OP_EQUAL).
@property(nonatomic, readonly) DMCScript* scriptHashScript;
...
@property(nonatomic, readonly) DMCScriptHashAddress* scriptHashAddress;
@property(nonatomic, readonly) DMCScriptHashAddressTestnet* scriptHashAddressTestnet;
...
// DEPRECATION WARNING: These methods were moved to DMCKey class.
