...
#define DMCBigNumberCompare(a, b) (BN_cmp(&(a->_bignum), &(b->_bignum)))

@implementation DMCBigNumber {
    @package
    BIGNUM _bignum;
...
- (void) throwIfImmutable {
    if (_immutable) {
        @throw [NSException exceptionWithName:@"Immutable DMCBigNumber is modified" reason:@"" userInfo:nil];
    }
}
...
- (DMCBigNumber*) copyWithZone:(NSZone *)zone {
    DMCBigNumber* to = [[DMCBigNumber alloc] init];
...
- (DMCMutableBigNumber*) mutableCopyWithZone:(NSZone *)zone {
    DMCMutableBigNumber* to = [[DMCMutableBigNumber alloc] init];
    if (BN_copy(&(to->_bignum), &_bignum)) {
        return to;
    }
    return nil;
...
- (BOOL) isEqual:(DMCBigNumber*)other {
    if (![other isKindOfClass:[DMCBigNumber class]]) return NO;
    return DMCBigNumberCompare(self, other) == NSOrderedSame;
}

- (NSComparisonResult)compare:(DMCBigNumber *)other {
    return DMCBigNumberCompare(self, other);
}
...
- (DMCBigNumber*) min:(DMCBigNumber*)other {
    return (DMCBigNumberCompare(self, other) <= 0) ? self : other;
}
...
- (BOOL) less:(DMCBigNumber *)other           { return DMCBigNumberCompare(self, other) <  0; }
- (BOOL) lessOrEqual:(DMCBigNumber *)other    { return DMCBigNumberCompare(self, other) <= 0; }
- (BOOL) greater:(DMCBigNumber *)other        { return DMCBigNumberCompare(self, other) >  0; }
- (BOOL) greaterOrEqual:(DMCBigNumber *)other { return DMCBigNumberCompare(self, other) >= 0; }
    ...
    if (BN_cmp(&bn, &bn0) == 0) {
        ...
        while (BN_cmp(&bn, &bn0) > 0) {
            if (!BN_div(&dv, &rem, &bn, &bnBase, pctx)) {
                NSLog(@"DMCBigNumber: stringInBase failed to BN_div");
                break;
            }
            BN_copy(&bn, &dv);
...
//
// DaemsCoin only uses this "compact" format for encoding difficulty
...
{
    BN_CTX* pctx = BN_CTX_new();
    DMCBigNumber* r = [DMCBigNumber new];
    DMCBigNumber* m = [DMCBigNumber new];
...
@implementation DMCMutableBigNumber
...
+ (instancetype) zero {
    return [[DMCBigNumber zero] mutableCopy];
}

+ (instancetype) one {
    return [[DMCBigNumber one] mutableCopy];
}

+ (instancetype) negativeOne {
    return [[DMCBigNumber negativeOne] mutableCopy];
}
...
- (instancetype) rshift:(unsigned int)shift { // >>=
    ...
    DMCMutableBigNumber* a = [DMCMutableBigNumber one];
    [a lshift:shift];
...
- (instancetype) exp:(DMCBigNumber*)power { // pow(self, p)
    BN_CTX* pctx = BN_CTX_new();
...
- (instancetype) exp:(DMCBigNumber*)power mod:(DMCBigNumber *)mod { // pow(self,p) % m
    BN_CTX* pctx = BN_CTX_new();
