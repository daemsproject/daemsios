...
extern NSString* const DMCTransactionBuilderErrorDomain;

typedef NS_ENUM(NSUInteger, DMCTransactionBuilderError) {

    // Change address is not specified.
    DMCTransactionBuilderChangeAddressMissing = 1,

    // No unspent outputs were provided or found.
    DMCTransactionBuilderUnspentOutputsMissing = 2,

    // Unspent outputs are not sufficient to build the transaction.
    DMCTransactionBuilderInsufficientFunds = 3,
};
...
@class DMCTransactionInput;
@class DMCTransactionOutput;
@class DMCTransactionBuilder;
@class DMCTransactionBuilderResult;

@protocol DMCTransactionBuilderDataSource <NSObject>

@required

// Called when needs inputs to spend in a transaction.
// DMCTransactionOutput instances must contain sensible `transactionHash` and `index` properties.
// Reference of DMCTransactionOutput is assigned to DMCTransactionInput so you could access it to sign the inputs.
...
- (NSEnumerator* /* [DMCTransactionOutput] */) unspentOutputsForTransactionBuilder:(DMCTransactionBuilder*)txbuilder;
...
- (DMCKey*) transactionBuilder:(DMCTransactionBuilder*)txbuilder keyForUnspentOutput:(DMCTransactionOutput*)txout;
...
- (DMCScript*) transactionBuilder:(DMCTransactionBuilder*)txbuilder signatureScriptForTransaction:(DMCTransaction*)tx script:(DMCScript*)outputScript inputIndex:(NSUInteger)inputIndex;
...
- (NSData*) shuffleSeedForTransactionBuilder:(DMCTransactionBuilder*)txbuilder;
...
@interface DMCTransactionBuilder : NSObject
...
@property(weak,nonatomic) id<DMCTransactionBuilderDataSource> dataSource;
...
// If failed to build a transaction, returns nil and sets error to one from DMCTransactionBuilderErrorDomain.
...
// Fee per 1000 bytes. Default is DMCTransactionDefaultFeeRate.
@property(nonatomic) DMCAmount feeRate;
...
@property(nonatomic) DMCAmount minimumChange;
...
// This means builder will never fail with DMCTransactionBuilderErrorInsufficientFunds just because it could not
...
@property(nonatomic) DMCAmount dustChange;
...
@interface DMCTransactionBuilderResult : NSObject
...
@property(nonatomic, readonly) DMCTransaction* transaction;

// Indexes of unsigned inputs. Such inputs have DMCTransactionOutput script in place of signatureScript.
// Also, every input has a reference to unspent DMCTransactionOutput provided by data source (or unspentOutputsEnumerator).
...
@property(nonatomic, readonly) DMCAmount fee;

// Complete amount on the inputs.
@property(nonatomic, readonly) DMCAmount inputsAmount;

// Complete amount on the outputs.
@property(nonatomic, readonly) DMCAmount outputsAmount;
