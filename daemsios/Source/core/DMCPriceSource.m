...
@implementation DMCPriceSourceResult
@end

@implementation DMCPriceSource
...
- (NSString*) name {
    [NSException raise:@"DMCPriceSource method not implemented" format:@"You must override -name method."];
    return nil;
}

// Supported currency codes ("USD", "EUR", "CNY" etc).
- (NSArray*) currencyCodes {
    [NSException raise:@"DMCPriceSource method not implemented" format:@"You must override -currencyCodes method."];
    return nil;
}

// Returns a NSURLRequest to fetch the avg price.
- (NSURLRequest*) requestForCurrency:(NSString*)currencyCode {
    [NSException raise:@"DMCPriceSource method not implemented" format:@"You must override either -loadPriceForCurrency:error: or -requestForCurrency:"];
    return nil;
}
...
- (DMCPriceSourceResult*) resultFromParsedData:(id)parsedData currencyCode:(NSString*)currencyCode error:(NSError**)errorOut {
    [NSException raise:@"DMCPriceSource method not implemented" format:@"You must override -resultFromParsedData:currencyCode:error: method."];
    return nil;
}
...
- (void) loadPriceForCurrency:(NSString*)currencyCode
            completionHandler:(void(^)(DMCPriceSourceResult* result, NSError* error))completionBlock {
...
- (void) loadPriceForCurrency:(NSString*)currencyCode
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        NSError* error = nil;
        DMCPriceSourceResult* result = [self loadPriceForCurrency:currencyCode error:&error];
...
- (DMCPriceSourceResult*) loadPriceForCurrency:(NSString*)currencyCode error:(NSError**)errorOut {
    ...
    if (![self.currencyCodes containsObject:currencyCode]) {
        NSError* error = [NSError errorWithDomain:DMCErrorDomain
                                             code:DMCErrorUnsupportedCurrencyCode
                                         userInfo:@{}];
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if (httpResponse.statusCode < 200 || httpResponse.statusCode >= 300) {
            error = [NSError errorWithDomain:DMCErrorDomain
    ...
    DMCPriceSourceResult* result = [self resultFromParsedData:parsedData currencyCode:currencyCode error:&error];
...
@implementation DMCPriceSourceCoindesk
...
- (DMCPriceSourceResult*) resultFromParsedData:(id)parsedData currencyCode:(NSString*)currencyCode error:(NSError**)errorOut {
    ...
    DMCPriceSourceResult* result = [[DMCPriceSourceResult alloc] init];
...
@implementation DMCPriceSourceWinkdex
...
@implementation DMCPriceSourceCoinbase
...
@implementation DMCPriceSourcePaymium
