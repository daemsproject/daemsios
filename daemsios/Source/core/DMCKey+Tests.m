...
#import "DMCKey+Tests.h"
#import "DMCKey.h"
#import "DMCAddress.h"
#import "NSData+DMCData.h"

@implementation DMCKey (Tests)
...
+ (void) runAllTests {
    ...
    [self testECDSA];
    [self testDaemsCoinSignedMessage];
}

+ (void) testRFC6979 {

    void(^verifyRFC6979)(NSString*, id, NSString*, NSString*) = ^(NSString* keyhex, id msg, NSString* khex, NSString* sighex) {
        NSData* hash = [msg isKindOfClass:[NSString class]] ? DMCSHA256([msg dataUsingEncoding:NSUTF8StringEncoding]) : msg;
        DMCKey* key = [[DMCKey alloc] initWithPrivateKey:DMCDataFromHex(keyhex)];
        NSData* k = [key signatureNonceForHash:hash];
        NSAssert([DMCDataFromHex(khex) isEqual:k], @"Must produce matching k nonce.");
        NSData* sig = [key signatureForHash:hash];
        NSAssert([DMCDataFromHex(sighex) isEqual:sig], @"Must produce matching signature.");
    };
    ...
    // Test from [DMCd/DMCec example](https://github.com/DMCsuite/DMCd/blob/master/DMCec/example_test.go)
    verifyRFC6979(@"22a47fa09a223f2aa079edf85a7c2d4f8720ee63e502ee2869afab7de234b80c",
                  DMCHash256([@"test message" dataUsingEncoding:NSUTF8StringEncoding]),
...
+ (void) testDiffieHellman {
    ...
    DMCKey* alice = [[DMCKey alloc] initWithPrivateKey:DMCDataFromHex(@"c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa73bd4e39a8a")];
    alice.publicKeyCompressed = YES;

    DMCKey* bob = [[DMCKey alloc] initWithPrivateKey:DMCDataFromHex(@"2db963f0fe106f483d9afa73bd4e39a8ac4bbcb1fbec99d65bf59d85c8cb62ee")];
    bob.publicKeyCompressed = YES;

    DMCKey* dh1 = [alice diffieHellmanWithPrivateKey:bob];
    DMCKey* dh2 = [bob diffieHellmanWithPrivateKey:alice];

    NSAssert([dh1.publicKey isEqual:dh2.publicKey], @"");
    NSAssert([dh1.publicKey isEqual:DMCDataFromHex(@"03735932754bc16e10febe40ee0280906d29459d477442f1838dcf27de3b5d9699")], @"");
    NSAssert([dh2.publicKey isEqual:DMCDataFromHex(@"03735932754bc16e10febe40ee0280906d29459d477442f1838dcf27de3b5d9699")], @"");
}

+ (void) testCanonicality
{
    {
        NSData* data = DMCDataFromHex(@"304402207f5561ac3cfb05743cab6ca914f7eb93c489f276f10cdf4549e7f0b0ef4e85cd02200191c0c2fd10f10158973a0344fdaf2438390e083a509d2870bcf2b05445612b01");

        NSError* error = nil;
        if (![DMCKey isCanonicalSignatureWithHashType:data verifyLowerS:YES error:&error]) {
            NSLog(@"error: %@", error);
        }
    }

    {
        NSData* data = DMCDataFromHex(@"3045022100e81a33ac22d0ef25d359a5353977f0f953608b2733141239ec02363237ab6781022045c71237e95b56079e9fa88591060e4c1a4bb02c0cad1ebeb092749d4aa9754701");
    ...
    {
        NSData* data = DMCDataFromHex(@"304402202692ad36ae12652c3f4bf068bd05477d867f654f2edf2cb15d335b25305d56b802206a4b51939b4b54fa62186e7bb78b4da8fe91475e5805897df11553dd1e08eb3e01");
...
+ (void) testRandomKeys {
    ...
    for (int i = 0; i < 32; i++) {
        DMCKey* k = [[DMCKey alloc] init];
        //NSLog(@"key = %@", DMCHexFromData(k.privateKey));
...
+ (void) testECDSA {
    for (int n = 0; n < 1000; n++) {
        ...
        DMCKey* key = [[DMCKey alloc] initWithPrivateKey:secret];
        key.publicKeyCompressed = YES;
        
        NSData* signature = [key signatureForHash:messageHash];
        
        DMCKey* key2 = [[DMCKey alloc] initWithPublicKey:key.publicKey];
...
+ (void) testBasicSigning {
    ...
    NSData* messageData = [message dataUsingEncoding:NSUTF8StringEncoding];
    NSData* secret = DMCDataFromHex(@"c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa73bd4e39a8a");
    ...
    DMCAddress* pubkeyAddress = [DMCPublicKeyAddress addressWithData:[key.publicKey DMCHash160]];
    DMCAddress* privkeyAddress = [DMCPrivateKeyAddress addressWithData:key.privateKey];
    ...
    DMCKey* key2 = [[DMCKey alloc] initWithPublicKey:pubkey];
...
+ (void) testDaemsCoinSignedMessage {
    NSString* message = @"Test message";
    NSData* secret = DMCDataFromHex(@"c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa73bd4e39a8a");
    DMCKey* key = [[DMCKey alloc] initWithPrivateKey:secret];
    key.publicKeyCompressed = NO;
    ...
    
    {
        ...
        //NSLog(@"Signature: %@ (%d bytes)", [signature hexString], (int)signature.length);
        DMCKey* key2 = [DMCKey verifySignature:signature forMessage:message];
    ...
    {
        NSData* signature = DMCDataFromHex(@"1B158259BD8EEB198BABBCC4308CDFB8E8068F0A712CAC634257933A072EA6DB7"
                                            "BEB3308F4C937D4F397A2A782BF12884045C27430719A2890F0127B4732D9CF0D");
        
        DMCKey* key = [DMCKey verifySignature:signature forMessage:@"Test message"];
