...
#import "DMCDaemsCoinURL+Tests.h"
#import "DMCAddress.h"

@implementation DMCDaemsCoinURL (Tests)
...
+ (void) testSimpleURL {
    DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:1JwSSubhmg6iPtRjtyqhUYYH7bZg3Lfy1T?amount=1.23450009&label=Hello%20world"]];
    NSAssert(burl, @"Must parse");
    NSAssert(burl.isValid == YES, @"Must be valid");
    NSAssert(burl.amount == 123450009, @"Must parse amount formatted as DMC");
...
+ (void) testCompatiblePaymentRequest {
    DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:1JwSSubhmg6iPtRjtyqhUYYH7bZg3Lfy1T?amount=1.23450009&r=http://example.com/order-1000123"]];
    NSAssert(burl, @"Must parse");
    NSAssert(burl.isValid == YES, @"Must be valid");
    NSAssert(burl.amount == 123450009, @"Must parse amount formatted as DMC");
    NSAssert([burl.address.string isEqual:@"1JwSSubhmg6iPtRjtyqhUYYH7bZg3Lfy1T"], @"Must parse address");
...
+ (void) testNakedPaymentRequest {
    DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:?r=http://example.com/order-1000123"]];
    NSAssert(burl, @"Must parse");
    NSAssert(burl.isValid == YES, @"Must be valid");
...
+ (void) testInvalidURL {
    {
    DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:?x=something"]];
    NSAssert(burl, @"Must parse");
    ...
    {
    DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:?amount=1.2"]];
    NSAssert(burl, @"Must parse");
    NSAssert(burl.isValid == NO, @"Must not be valid");
...
+ (void) testMalformedURL {
    {
        DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"daemsCoin:xxxx"]];
        NSAssert(!burl, @"Must not parse broken address");
    }
    {
        DMCDaemsCoinURL* burl = [[DMCDaemsCoinURL alloc] initWithURL:[NSURL URLWithString:@"http://example.com"]];
        NSAssert(!burl, @"Must not parse other schemas than daemsCoin:");
    }
}
@end
