...
// Messages are encrypted with recipient's public key (DaemsCoin-compatible, that is secp256k1)
...
// 1 byte      proof-of-work difficulty target (similar to "bits" field in DMCBlockHeader). SHA512^2 of the entire encrypted message should be below target.
...
// R bits      recipient's address (DMCHash160(pubkey)), from 0 to 20 bytes. Only first R bits are used.
...
// 1,2,3,5,9 bytes of message length (M) as 'CompactSize' variable-length integer encoding. See DMCProtocolSerialization.
...
static unsigned char DMCFancyEncryptedMessageVersion[4] = {0xc1, 0xb9, 0xf0, 0xe3};

typedef NS_ENUM(uint8_t, DMCFancyEncryptedMessageAddressLength) {
    // No address is given, recipient must attempt to decrypt the message to figure if he is indeed a recipient.
    DMCFancyEncryptedMessageAddressLengthNone           = 0,
    
    // First 4 bits of the address. To route message efficiently to 6% of all users yet keeping recipient secret.
    DMCFancyEncryptedMessageAddressLengthLightRouting   = 4,
    ...
    DMCFancyEncryptedMessageAddressLengthNormalRouting  = 7,
    
    // Fingerprint is a short 32-bit identifier to be compact, but yet allow fast identification like in BIP32 / DMCKeychain.
    DMCFancyEncryptedMessageAddressLengthFingerprint    = 32,
    
    // Full 160-bit address fully identifying the recipient.
    DMCFancyEncryptedMessageAddressLengthFull           = 160,
};

@class DMCKey;
@interface DMCFancyEncryptedMessage : NSObject
...
- (NSData*) encryptedDataWithKey:(DMCKey*)recipientKey seed:(NSData*)seed;
