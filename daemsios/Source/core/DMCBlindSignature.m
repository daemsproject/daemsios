...
@implementation DMCBlindSignature {
    DMCKeychain* _clientKeychain;
    DMCKeychain* _custodianKeychain;
}
...
- (id) initWithClientKeychain:(DMCKeychain*)clientKeychain custodianKeychain:(DMCKeychain*)custodianKeychain {
...
- (id) initWithCustodianKeychain:(DMCKeychain*)custodianKeychain {
...
- (DMCKey*) publicKeyAtIndex:(uint32_t)index {
    DMCBigNumber* a = [self privateNumberFromKeychain:_clientKeychain index:4*index + 0];
    DMCBigNumber* b = [self privateNumberFromKeychain:_clientKeychain index:4*index + 1];
    DMCBigNumber* c = [self privateNumberFromKeychain:_clientKeychain index:4*index + 2];
    DMCBigNumber* d = [self privateNumberFromKeychain:_clientKeychain index:4*index + 3];
    
    DMCCurvePoint* P = [self pointFromKeychain:_custodianKeychain index:2*index + 0];
    DMCCurvePoint* Q = [self pointFromKeychain:_custodianKeychain index:2*index + 1];
    ...
    DMCKey* key = [[DMCKey alloc] initWithCurvePoint:T];
...
- (NSData*) blindedHashForHash:(NSData*)hash index:(uint32_t)index {
    ...
    DMCBigNumber* h1 = [[DMCBigNumber alloc] initWithUnsignedBigEndian:hash];
    
    DMCBigNumber* h2 = [self aliceBlindedHashForHash:h1 a:a b:b];
...
- (NSData*) blindSignatureForBlindedHash:(NSData*)blindedHash {
    ...
    DMCBigNumber* h2 = [[DMCBigNumber alloc] initWithUnsignedBigEndian:[blindedHash subdataWithRange:NSMakeRange(4, blindedHash.length - 4)]];
    ...
    DMCBigNumber* w = [[DMCBigNumber alloc] initWithUnsignedBigEndian:_custodianKeychain.key.privateKey];
    ...
    DMCBigNumber* p = [[[w mutableCopy] add:x mod:order] inverseMod:order];
    DMCBigNumber* q = [[[w mutableCopy] add:y mod:order] multiply:p mod:order];
    
    DMCBigNumber* s1 = [self bobBlindedSignatureForHash:h2 p:p q:q];
...
// Note: Do not forget to add SIGHASH byte in the end when placing in a DaemsCoin transaction.
...
- (NSData*) unblindedSignatureForBlindSignature:(NSData*)blindSignature verifyHash:(NSData*)hashToVerify {
    ...
    DMCBigNumber* s1 = [[DMCBigNumber alloc] initWithUnsignedBigEndian:[blindSignature subdataWithRange:NSMakeRange(4, blindSignature.length - 4)]];
    ...
    DMCBigNumber* s2 = [self aliceUnblindedSignatureForSignature:s1 c:c d:d];
    ...
    // Verify signature
    if (hashToVerify) {
        DMCKey* pubkey = [self publicKeyAtIndex:index];
...
- (DMCBigNumber*) privateNumberFromKeychain:(DMCKeychain*)keychain index:(uint32_t)index {
    DMCKeychain* derivedKC = [keychain derivedKeychainAtIndex:index hardened:YES];
    
    NSAssert(derivedKC, @"sanity check");
    
    DMCKey* key = derivedKC.key;
    ...
    DMCBigNumber* bn = [[DMCBigNumber alloc] initWithUnsignedBigEndian:privKeyData];
...
- (DMCCurvePoint*) pointFromKeychain:(DMCKeychain*)keychain index:(uint32_t)index {
    DMCKeychain* derivedKC = [keychain derivedKeychainAtIndex:index hardened:NO];
    ...
    DMCCurvePoint* point = [key curvePoint];
...
//    Thus, Alice can safely publish T (e.g. in a DaemsCoin transaction that locks funds with T).
// 4. When time comes to sign a message (e.g. redeeming funds locked in a DaemsCoin transaction),
...
//    If she uses it in a DaemsCoin transaction, she will be able to redeem her locked funds without Bob knowing which transaction he just helped to sign.

// Step 2: P and Q from Bob as blinded "public keys". Both P and C are DMCCurvePoint instances.
- (NSArray*) bob_P_and_Q_for_p:(DMCBigNumber*)p q:(DMCBigNumber*)q {
    if (!p || !q) return nil;
    
    DMCCurvePoint* P = nil;
    DMCCurvePoint* Q = nil;
    DMCBigNumber* invp = nil;
    
    invp = [[p mutableCopy] inverseMod:[DMCCurvePoint curveOrder]];
    
    P = [[DMCCurvePoint generator] multiply:invp];
    Q = [[P copy] multiply:q];
...
// 3. Alice computes K = (c·a)^-1·P and public key T = (a·Kx)^-1·(b·G + Q + d·c^-1·P).
//    Bob cannot know if his parameters were involved in K or T without the knowledge of a, b, c and d.
- (NSArray*) alice_K_and_T_for_a:(DMCBigNumber*)a b:(DMCBigNumber*)b c:(DMCBigNumber*)c d:(DMCBigNumber*)d P:(DMCCurvePoint*)P Q:(DMCCurvePoint*)Q {
    ...
    // Temporary vars
    DMCBigNumber* curveOrder = [DMCCurvePoint curveOrder];
    DMCBigNumber* invc = nil;
    DMCBigNumber* inva = nil;
    DMCBigNumber* Kx = nil;
    DMCBigNumber* invKx = nil;
...
- (DMCBigNumber*) linearTransformOfNumber:(DMCBigNumber*)number multiply:(DMCBigNumber*)a add:(DMCBigNumber*)b {
    if (!number || !a || !b) return nil;
    
    DMCBigNumber* curveOrder = [DMCCurvePoint curveOrder];
    DMCMutableBigNumber* result = [[[number mutableCopy] multiply:a mod:curveOrder] add:b mod:curveOrder];
    return result;
}

// 5. Alice blinds the hash and sends h2 = a·h + b (mod n) to Bob.
- (DMCBigNumber*) aliceBlindedHashForHash:(DMCBigNumber*)hash a:(DMCBigNumber*)a b:(DMCBigNumber*)b {
    return [self linearTransformOfNumber:hash multiply:a add:b];
}

// 7. Bob signs the blinded hash and returns the signature to Alice: s1 = p·h2 + q (mod n).
- (DMCBigNumber*) bobBlindedSignatureForHash:(DMCBigNumber*)hash p:(DMCBigNumber*)p q:(DMCBigNumber*)q {
    return [self linearTransformOfNumber:hash multiply:p add:q];
}

// 8. Alice unblinds the signature: s2 = c·s1 + d (mod n).
- (DMCBigNumber*) aliceUnblindedSignatureForSignature:(DMCBigNumber*)blindSignature c:(DMCBigNumber*)c d:(DMCBigNumber*)d {
...
// 0x01 - hash type
- (NSData*) aliceCompleteSignatureForKx:(DMCBigNumber*)Kx unblindedSignature:(DMCBigNumber*)unblindedSignature {
    ECDSA_SIG sigValue;
    ...
    // The remaining code is taken from DMCKey where we produce a canonical signature.
