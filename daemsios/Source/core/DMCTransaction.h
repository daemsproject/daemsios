...
static const uint32_t DMCTransactionCurrentVersion = 1;
static const DMCAmount DMCTransactionDefaultFeeRate = 10000; // 10K satoshis per 1000 bytes
 ...
 */
NSData* DMCTransactionHashFromID(NSString* txid) DEPRECATED_ATTRIBUTE;

/*!
 * Converts hash of the transaction to its string ID (reversed tx hash in hex format).
 */
NSString* DMCTransactionIDFromHash(NSData* txhash) DEPRECATED_ATTRIBUTE;


/*!
 * DMCTransaction represents a DaemsCoin transaction structure which contains
 * inputs, outputs and additional metadata.
 */
@interface DMCTransaction : NSObject<NSCopying>
...
// Array of DMCTransactionInput objects
@property(nonatomic) NSArray* inputs;

// Array of DMCTransactionOutput objects
...
@property(nonatomic, readonly) DMCAmount estimatedFee;
