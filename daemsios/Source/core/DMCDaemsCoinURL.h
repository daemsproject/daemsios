...
/*!
 * Class to compose and handle various DaemsCoin URLs according to BIP21.
 * See: https://github.com/daemsCoin/bips/blob/master/bip-0021.mediawiki
 */
@class DMCAddress;
@class DMCAssetID;
@interface DMCDaemsCoinURL : NSObject

/*!
 * Encoded address.
 */
@property(nonatomic, nullable) DMCAddress* address;
 ...
 */
@property(nonatomic, nullable) DMCAssetID* assetID;
...
/*!
 * Returns YES if it is a pure daemsCoin URL. That does not specify asset_id and not uses openassets: scheme.
 */
@property(nonatomic, readonly) BOOL isValidDaemsCoinURL;
...
/*!
 * Makes a URL in form "daemsCoin:<address>?amount=1.2345&label=<label>.
 * @param address Address to be rendered in base58 format.
 * @param amount  Amount in satoshis. Note that URI scheme dictates to render this amount as a decimal number in DMC.
 * @param label   Optional label.
 */
+ (nonnull NSURL*) URLWithAddress:(nonnull DMCAddress*)address amount:(DMCAmount)amount label:(nullable NSString*)label;

/*!
 * Instantiates if URL is a valid daemsCoin: URL.
...
/*!
 * Instantiates an empty DaemsCoin URL.
