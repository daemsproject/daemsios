...
@interface DMCScriptMachine ()

// Constants
@property(nonatomic) NSData* blobFalse;
@property(nonatomic) NSData* blobZero;
@property(nonatomic) NSData* blobTrue;
@property(nonatomic) DMCBigNumber* bigNumberZero;
@property(nonatomic) DMCBigNumber* bigNumberOne;
@property(nonatomic) DMCBigNumber* bigNumberFalse;
@property(nonatomic) DMCBigNumber* bigNumberTrue;
@end
...
@implementation DMCScriptMachine {
...
- (id) init {
    if (self = [super init]) {
        ...
        _bigNumberZero = [[DMCBigNumber alloc] initWithInt32:0];
        _bigNumberOne = [[DMCBigNumber alloc] initWithInt32:1];
...
- (id) initWithTransaction:(DMCTransaction*)tx inputIndex:(uint32_t)inputIndex {
    if (!tx) return nil;
...
- (id) copyWithZone:(NSZone *)zone {
    DMCScriptMachine* sm = [[DMCScriptMachine alloc] init];
...
- (BOOL) shouldVerifyP2SH {
    return (_blockTimestamp >= DMC_BIP16_TIMESTAMP);
}

- (BOOL) verifyWithOutputScript:(DMCScript*)outputScript error:(NSError**)errorOut {
    // self.inputScript allows to override transaction so we can simply testing.
    DMCScript* inputScript = self.inputScript;
    ...
    if (!inputScript) {
        ...
        if (!(self.transaction && self.inputIndex < self.transaction.inputs.count)) {
            [NSException raise:@"DMCScriptMachineException"  format:@"transaction and valid inputIndex are required for script verification."];
            return NO;
        }
        if (!outputScript) {
            [NSException raise:@"DMCScriptMachineException"  format:@"non-nil outputScript is required for script verification."];
            return NO;
        }

        DMCTransactionInput* txInput = self.transaction.inputs[self.inputIndex];
    ...
    if (shouldVerifyP2SH) {
        ...
        if (stackForP2SH.count == 0) {
            ...
            [NSException raise:@"DMCScriptMachineException"  format:@"internal inconsistency: stackForP2SH cannot be empty at this point."];
            return NO;
        }
        
        // Instantiate the script from the last data on the stack.
        DMCScript* providedScript = [[DMCScript alloc] initWithData:[stackForP2SH lastObject]];
...
- (BOOL) runScript:(DMCScript*)script error:(NSError**)errorOut {
    if (!script) {
        [NSException raise:@"DMCScriptMachineException"  format:@"non-nil script is required for -runScript:error: method."];
        return NO;
    }
    
    if (script.data.length > DMC_MAX_SCRIPT_SIZE) {
    ...
    [script enumerateOperations:^(NSUInteger opIndex, DMCOpcode opcode, NSData *pushdata, BOOL *stop) {
...
- (BOOL) executeOpcodeError:(NSError**)errorOut {
    ...
    DMCOpcode opcode = _opcode;
    NSData* pushdata = _pushdata;
    
    if (pushdata.length > DMC_MAX_SCRIPT_ELEMENT_SIZE) {
        if (errorOut) *errorOut = [self scriptError:NSLocalizedString(@"Pushdata chunk size is too big.", @"")];
        return NO;
    }
    
    if (opcode > OP_16 && !_pushdata && ++_opCount > DMC_MAX_OPS_PER_SCRIPT) {
    ...
    // this basically means that OP_VERIF and OP_VERNOTIF will always fail the script, even if not executed.
        switch (opcode) {
            ...
            case OP_16: {
                // ( -- value)
                DMCBigNumber* bn = [[DMCBigNumber alloc] initWithInt64:(int)opcode - (int)(OP_1 - 1)];
            ...
            case OP_FROMALTSTACK: {
                if (_altStack.count < 1) {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"%@ requires one item on altstack", @""), DMCNameForOpcode(opcode)]];
                    return NO;
                }
            ...
            case OP_DEPTH: {
                // -- stacksize
                DMCBigNumber* bn = [[DMCBigNumber alloc] initWithInt64:_stack.count];
                [_stack addObject:bn.signedLittleEndian];
            }
            break;
            ...
            case OP_ROLL: {
                ...
                DMCBigNumber* bn = [self bigNumberAtIndex:-1];
                ...
                if (n < 0 || n >= _stack.count) {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of items for %@: %d.", @""), DMCNameForOpcode(opcode), n]];
                    return NO;
                }
            ...
            case OP_SIZE: {
                ...
                DMCBigNumber* bn = [[DMCBigNumber alloc] initWithUInt64:[self dataAtIndex:-1].length];
                [_stack addObject:bn.signedLittleEndian];
            }
            break;
            ...
            case OP_0NOTEQUAL: {
                ...
                DMCMutableBigNumber* bn = [self bigNumberAtIndex:-1];
                ...
                switch (opcode) {
                    ...
                    case OP_NEGATE:     [bn multiply:[DMCBigNumber negativeOne]]; break;
                    case OP_ABS:        if ([bn less:_bigNumberZero]) [bn multiply:[DMCBigNumber negativeOne]]; break;
            ...
            case OP_MAX: {
                ...
                DMCMutableBigNumber* bn1 = [self bigNumberAtIndex:-2];
                DMCMutableBigNumber* bn2 = [self bigNumberAtIndex:-1];
                ...
                switch (opcode) {
                    ...
                    case OP_BOOLAND:             bn = [[DMCMutableBigNumber alloc] initWithInt32:![bn1 isEqual:_bigNumberZero] && ![bn2 isEqual:_bigNumberZero]]; break;
                    case OP_BOOLOR:              bn = [[DMCMutableBigNumber alloc] initWithInt32:![bn1 isEqual:_bigNumberZero] || ![bn2 isEqual:_bigNumberZero]]; break;
                    case OP_NUMEQUAL:            bn = [[DMCMutableBigNumber alloc] initWithInt32: [bn1 isEqual:bn2]]; break;
                    case OP_NUMEQUALVERIFY:      bn = [[DMCMutableBigNumber alloc] initWithInt32: [bn1 isEqual:bn2]]; break;
                    case OP_NUMNOTEQUAL:         bn = [[DMCMutableBigNumber alloc] initWithInt32:![bn1 isEqual:bn2]]; break;
                    case OP_LESSTHAN:            bn = [[DMCMutableBigNumber alloc] initWithInt32:[bn1 less:bn2]]; break;
                    case OP_GREATERTHAN:         bn = [[DMCMutableBigNumber alloc] initWithInt32:[bn1 greater:bn2]]; break;
                    case OP_LESSTHANOREQUAL:     bn = [[DMCMutableBigNumber alloc] initWithInt32:[bn1 lessOrEqual:bn2]]; break;
                    case OP_GREATERTHANOREQUAL:  bn = [[DMCMutableBigNumber alloc] initWithInt32:[bn1 greaterOrEqual:bn2]]; break;
            ...
            case OP_WITHIN: {
                ...
                DMCMutableBigNumber* bn1 = [self bigNumberAtIndex:-3];
                DMCMutableBigNumber* bn2 = [self bigNumberAtIndex:-2];
                DMCMutableBigNumber* bn3 = [self bigNumberAtIndex:-1];
            ...
            case OP_HASH256: {
                ...
                if (opcode == OP_RIPEMD160) {
                    hash = DMCRIPEMD160(data);
                } else if (opcode == OP_SHA1) {
                    hash = DMCSHA1(data);
                } else if (opcode == OP_SHA256) {
                    hash = DMCSHA256(data);
                } else if (opcode == OP_HASH160) {
                    hash = DMCHash160(data);
                } else if (opcode == OP_HASH256) {
                    hash = DMCHash256(data);
                }
                [self popFromStack];
            ...
            case OP_CODESEPARATOR: {
                ...
                // DaemsCoind will extract subscript *including* this codeseparator. But all codeseparators will be stripped out eventually
                // when we compute a hash of transaction. Just to keep ourselves close to daemsCoind for extra asfety, we'll do the same here.
            ...
            case OP_CHECKSIGVERIFY: {
                ...
                DMCScript* subscript = [_script subScriptFromIndex:_lastCodeSeparatorIndex];

                // Drop the signature, since there's no way for a signature to sign itself.
                // Normally we neither have signatures in the output scripts, nor checksig ops in the input scripts.
                // In early days of DaemsCoin (before July 2010) input and output scripts were concatenated and executed as one,
                ...
                // See https://daemsCointalk.org/index.php?topic=278992.0 for more info.
                [subscript deleteOccurrencesOfData:signature];

                NSError* sigerror = nil;
                BOOL failed = NO;
                if (_verificationFlags & DMCScriptVerificationStrictEncoding) {
                    if (![DMCKey isCanonicalPublicKey:pubkeyData error:&sigerror]) {
                        failed = YES;
                    }
                    if (!failed && ![DMCKey isCanonicalSignatureWithHashType:signature
                                                                 verifyLowerS:!!(_verificationFlags & DMCScriptVerificationEvenS)
                                                                       error:&sigerror]) {
                        failed = YES;
                    }
                }
            ...
            case OP_CHECKMULTISIGVERIFY: {
                ...
                DMCBigNumber* bn = [self bigNumberAtIndex:-i];
                ...
                if (keysCount < 0 || keysCount > DMC_MAX_KEYS_FOR_CHECKMULTISIG) {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of keys for %@: %d.", @""), DMCNameForOpcode(opcode), keysCount]];
                    return NO;
                }
                
                _opCount += keysCount;
                
                if (_opCount > DMC_MAX_OPS_PER_SCRIPT) {
                ...
                DMCBigNumber* bn2 = [self bigNumberAtIndex:-i];
                ...
                if (sigsCount < 0 || sigsCount > keysCount) {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of signatures for %@: %d.", @""), DMCNameForOpcode(opcode), keysCount]];
                    return NO;
                }
                ...
                while (success && sigsCount > 0) {
                    ...
                    if (_verificationFlags & DMCScriptVerificationStrictEncoding) {
                        ...
                        if (validMatch && ![DMCKey isCanonicalSignatureWithHashType:signature
                                                                        verifyLowerS:!!(_verificationFlags & DMCScriptVerificationEvenS)
                                                                              error:&sigerror]) {
                            validMatch = NO;
                        }
            ...
            default:
                if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Unknown opcode %d (%@).", @""), opcode, DMCNameForOpcode(opcode)]];
                return NO;
        }
    }
...
- (BOOL) checkSignature:(NSData*)signature publicKey:(NSData*)pubkeyData subscript:(DMCScript*)subscript error:(NSError**)errorOut {
    DMCKey* pubkey = [[DMCKey alloc] initWithPublicKey:pubkeyData];
    
    if (!pubkey) {
        if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Public key is not valid: %@.", @""),
                                                     DMCHexFromData(pubkeyData)]];
        return NO;
    }
    ...
    DMCSignatureHashType hashType = ((unsigned char*)signature.bytes)[signature.length - 1];
    ...
    //NSLog(@"DMCScriptMachine: Hash for input %d [%d]: %@", _inputIndex, hashType, DMCHexFromData(sighash));
...
- (NSError*) scriptError:(NSString*)localizedString {
    return [NSError errorWithDomain:DMCErrorDomain
...
- (NSError*) scriptErrorOpcodeRequiresItemsOnStack:(NSUInteger)items {
    if (items == 1) {
        return [NSError errorWithDomain:DMCErrorDomain
                                   code:DMCErrorScriptError
                               userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %d item on stack.", @""), DMCNameForOpcode(_opcode), items]}];
    }
    return [NSError errorWithDomain:DMCErrorDomain code:DMCErrorScriptError userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %d items on stack.", @""), DMCNameForOpcode(_opcode), items]}];
}
...
#define DMCNormalizeIndex(list, i) (i < 0 ? (list.count + i) : i)

- (NSData*) dataAtIndex:(NSInteger)index {
    return _stack[DMCNormalizeIndex(_stack, index)];
}

- (void) swapDataAtIndex:(NSInteger)index1 withIndex:(NSInteger)index2 {
    [_stack exchangeObjectAtIndex:DMCNormalizeIndex(_stack, index1)
                withObjectAtIndex:DMCNormalizeIndex(_stack, index2)];
}

// Returns bignum from pushdata or nil.
- (DMCMutableBigNumber*) bigNumberAtIndex:(NSInteger)index {
    ...
    DMCMutableBigNumber* bn = [[DMCMutableBigNumber alloc] initWithSignedLittleEndian:[[DMCBigNumber alloc] initWithSignedLittleEndian:data].signedLittleEndian];
    return bn;
}
...
- (void) removeAtIndex:(NSInteger)index {
    [_stack removeObjectAtIndex:DMCNormalizeIndex(_stack, index)];
}

// -1 means last item
- (void) popFromStack {
    [_stack removeObjectAtIndex:DMCNormalizeIndex(_stack, -1)];
}
