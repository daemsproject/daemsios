...
#import "DMCErrors.h"
#import "DMCFancyEncryptedMessage.h"
#import "DMCData.h"
#import "DMCKey.h"
...
#define DMCFancyEncryptedMessageChecksumLength 16

static uint8_t DMCEMCompactTargetForFullTarget(uint32_t fullTarget);
static uint32_t DMCEMFullTargetForCompactTarget(uint8_t compactTarget);

@implementation DMCFancyEncryptedMessage {
...
- (id) initWithData:(NSData*)data {
    ...
    if (self = [super init]) {
        ...
        _addressLength = DMCFancyEncryptedMessageAddressLengthNone;
        _decryptedData = data;
    }
    return self;
}

- (NSData*) encryptedDataWithKey:(DMCKey*)recipientKey seed:(NSData*)seed0 {
    ...
    self.address = DMCHash160(recipientPubKey);
    ...
    if (seed0) {
        ...
        void* randomBytes = DMCCreateRandomBytesOfLength(32);
        CC_SHA256_Update(&ctx256, randomBytes, 32);
        DMCSecureMemset(randomBytes, 0, 32);
        free(randomBytes);
    }
    ...
    DMCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
    DMCDataClear(recipientPubKey);
    
    if (self.timestampVariance > 0) {
        NSData* varianceHash = DMCSHA256(seed); // extra hashing to not leak our seed that we'll use for private key nonce.
        uint32_t rand = *((uint32_t*)varianceHash.bytes);
        self.timestamp -= (rand % self.timestampVariance);
    }
    
    uint8_t compactTarget = DMCEMCompactTargetForFullTarget(_difficultyTarget);
    ...
    do {
        ...
        DMCDataClear(messageData); // clear previous trial
        
        [messageData setLength:0];
        
        [messageData appendBytes:DMCFancyEncryptedMessageVersion length:4];
        ...
        DMCKey* nonceKey = [[DMCKey alloc] initWithPrivateKey:privkey];
        ...
        DMCBigNumber* pk = [[DMCBigNumber alloc] initWithUnsignedBigEndian:privkey];
        
        DMCCurvePoint* curvePoint = recipientKey.curvePoint;
        ...
        if (cryptstatus != kCCSuccess) {
            DMCDataClear(encryptedData);
            DMCSecureMemset(&ctx256, 0, sizeof(ctx256));
            encryptedData = nil;
        } else {
        ...
        if ([pointX isKindOfClass:[NSMutableData class]]) {
            DMCDataClear((NSMutableData*)pointX);
        }
        [pk clear];
        [curvePoint clear];
        DMCSecureMemset(&ctx256, 0, sizeof(ctx256));
        ...
        [messageData appendData:[DMCProtocolSerialization dataForVarString:encryptedData]];
        ...
        [messageData appendBytes:digest256 length:DMCFancyEncryptedMessageChecksumLength];
...
- (id) initWithEncryptedData:(NSData*)data {
    if (self = [super init]) {
        ...
        if (datalength < (4 + 1 + 1 + 4 + 1 + 1 + 32 + 1 + DMCFancyEncryptedMessageChecksumLength)) return nil;
        ...
        if (memcmp(DMCFancyEncryptedMessageVersion, msgbytes, sizeof(DMCFancyEncryptedMessageVersion)) != 0) {
            return nil;
        }
        ...
        uint32_t target = DMCEMFullTargetForCompactTarget(msgbytes[4]);
        ...
        DMCKey* nonceKey = [[DMCKey alloc] initWithPublicKey:pubkeyNonceData];
        ...
        _encryptedData = [DMCProtocolSerialization readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, datalength - offset)] readBytes:&encodedMessageLength];
        
        if (_encryptedData == nil) return nil;
        
        offset += encodedMessageLength;
        
        if (datalength < offset + DMCFancyEncryptedMessageChecksumLength) return nil;
        
        _checksum = [data subdataWithRange:NSMakeRange(offset, DMCFancyEncryptedMessageChecksumLength)];
...
- (NSData*) decryptedDataWithKey:(DMCKey*)key error:(NSError**)errorOut {
    ...
    DMCBigNumber* pk = [[DMCBigNumber alloc] initWithUnsignedBigEndian:key.privateKey];
    
    DMCCurvePoint* curvePoint = _nonceKey.curvePoint;
    ...
    if (_checksum.length != DMCFancyEncryptedMessageChecksumLength) {
        DMCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
    
    if (memcmp(digest256, _checksum.bytes, DMCFancyEncryptedMessageChecksumLength) != 0) {
        DMCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
...
static uint8_t DMCEMCompactTargetForFullTarget(uint32_t fullTarget) {
    ...
    for (uint8_t ct = 0xFF; ct >= 0; --ct) {
        if (DMCEMFullTargetForCompactTarget(ct) <= fullTarget) {
            uint32_t order = ct >> 3;
