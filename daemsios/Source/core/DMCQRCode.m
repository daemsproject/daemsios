...
#if TARGET_OS_IPHONE
@interface DMCQRCodeScannerView : UIView <AVCaptureMetadataOutputObjectsDelegate>
...
+ (UIView*) scannerViewUsingDevice:(AVCaptureDevicePosition) devicePosition
    ...
    DMCQRCodeScannerView *view = [[DMCQRCodeScannerView alloc] initWithDetectionBlock:detectionBlock];
...
- (void) prepareScannerWithDevice:(AVCaptureDevice *)device {
    ...
    if (!input) {
        NSLog(@"DMCQRCodeScannerView: Failed to instantiate a video device: %@", [error localizedDescription]);
        return;
    }
    ...
    if ([device lockForConfiguration:&error]) {
        ...
        NSLog(@"DMCQRCodeScannerView: Failed to lock device for configuration: %@", [error localizedDescription]);
    }
    ...
    if ([output.availableMetadataObjectTypes containsObject:AVMetadataObjectTypeQRCode]) {
        ...
        NSLog(@"DMCQRCodeScannerView: QRCode not found in availableMetadataObjectTypes: %@", output.availableMetadataObjectTypes);
        [self cleanup];
        return;
    }
