...
@interface DMCEncryptedBackup ()
@property(nonatomic, readwrite) DMCEncryptedBackupVersion version;
...
@implementation DMCEncryptedBackup

- (id) initWithBackupKey:(NSData*)backupKey {
    if (!backupKey) return nil;
    if (self = [super init]) {
        self.version = DMCEncryptedBackupVersion1;
...
+ (instancetype) encrypt:(NSData*)data backupKey:(NSData*)backupKey timestamp:(NSTimeInterval)timestamp {
    DMCEncryptedBackup* b = [[DMCEncryptedBackup alloc] initWithBackupKey:backupKey];
...
+ (NSData*) backupKeyForNetwork:(DMCNetwork*)network masterKey:(NSData*)masterKey {
    if (!network || network.isMainnet) {
        return DMCHMACSHA256(masterKey, [@"Automatic Backup Key Mainnet" dataUsingEncoding:NSASCIIStringEncoding]);
    } else {
        return DMCHMACSHA256(masterKey, [@"Automatic Backup Key Testnet" dataUsingEncoding:NSASCIIStringEncoding]);
    }
}

+ (DMCKey*) authenticationKeyWithBackupKey:(NSData*)backupKey {
    DMCKey* key = [[DMCKey alloc] initWithPrivateKey:DMCHMACSHA256(backupKey, [@"Authentication Key" dataUsingEncoding:NSUTF8StringEncoding])];
...
+ (NSString*) walletIDWithAuthenticationKey:(NSData*)authPubkey {
    ...
    [data appendData:DMCHash160(authPubkey)];
    return DMCBase58CheckStringWithData(data);
}


- (NSData*) encryptionKey {
    return [DMCHMACSHA256(self.backupKey, [@"Encryption Key" dataUsingEncoding:NSUTF8StringEncoding]) subdataWithRange:NSMakeRange(0, 16)];
}
...
- (NSData*) encrypt {
    ...
    DMCKey* ak = [self authenticationKey];
    ...
    [result appendData:[DMCProtocolSerialization dataForVarString:ciphertext]];
    [result appendData:[DMCProtocolSerialization dataForVarString:signature]];
    return result;
}
...
- (NSData*) decrypt {
    ...
    NSUInteger ctlen = 0;
    NSData* ciphertext = [DMCProtocolSerialization readVarStringFromData:[payload subdataWithRange:NSMakeRange(fixedOffset, payload.length - fixedOffset)] readBytes:&ctlen];

    if (!ciphertext) {
        return nil;
    }

    NSData* signature = [DMCProtocolSerialization readVarStringFromData:
    ...
    if (![ak isValidSignature:signature hash:DMCHash256(sigData)]) {
        return nil;
    }
...
- (NSData*) ivForPlaintext:(NSData*)plaintext ek:(NSData*)ek {
    return [DMCHMACSHA256(ek, plaintext) subdataWithRange:NSMakeRange(0, 16)];
}
...
- (NSData*) merkleRootForCiphertext:(NSData*)data {
    ...
    */
    ...
    DMCMerkleTree* mt = [[DMCMerkleTree alloc] initWithDataItems:dataItems];
    return mt.merkleRoot;
}

- (NSData*) dataForSigning:(DMCEncryptedBackupVersion)version timestamp:(NSTimeInterval)timestamp iv:(NSData*)iv merkleRoot:(NSData*)merkleRoot {
