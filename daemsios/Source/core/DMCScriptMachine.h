...
typedef NS_ENUM(NSUInteger, DMCScriptVerification) {
    DMCScriptVerificationStrictEncoding = (1U << 1), // enforce strict conformance to DER and SEC2 for signatures and pubkeys (aka SCRIPT_VERIFY_STRICTENC)
    DMCScriptVerificationEvenS          = (1U << 2), // enforce lower S values (below curve halforder) in signatures (aka SCRIPT_VERIFY_EVEN_S, depends on STRICTENC)
};
...
@interface DMCScriptMachine : NSObject <NSCopying>

// "To" transaction that is signed by an inputScript.
// Required parameter.
@property(nonatomic) DMCTransaction* transaction;
...
@property(nonatomic) DMCScript* inputScript;
...
// Defaults in DaemsCoin: be liberal in what you accept and conservative in what you send.
// So we try to create canonical purist transactions but have no problem accepting and working with non-canonical ones.
@property(nonatomic) DMCScriptVerification verificationFlags;
