...
#import "DMCKeychain+Tests.h"
#import "DMCData.h"
#import "DMCBase58.h"
#import "DMCKey.h"
#import "DMCAddress.h"

@implementation DMCKeychain (Tests)
...
+ (void) testPaths {
    DMCKeychain* keychain = [[DMCKeychain alloc] initWithExtendedKey:@"xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi"];
...
+ (void) testStandardTestVectors {
     ...
     */
    {
        NSData* seed = DMCDataWithHexCString("000102030405060708090a0b0c0d0e0f");
        
        DMCKeychain* masterChain = [[DMCKeychain alloc] initWithSeed:seed];
        ...
        NSAssert([masterChain.identifier isEqual:DMCDataFromHex(@"3442193e1bb70916e914552172cd4e2dbc9df811")], @"");
        ...
        DMCKeychain* m0prv = [masterChain derivedKeychainWithPath:@"m/0'"];
        ...
        DMCKeychain* m0prv1pub = [masterChain derivedKeychainWithPath:@"m/0'/1"];
        ...
        DMCKeychain* m0prv1pub2prv = [masterChain derivedKeychainWithPath:@"m/0'/1/2'"];
        ...
        DMCKeychain* m0prv1pub2prv2pub = [m0prv1pub2prv derivedKeychainAtIndex:2];
        ...
        DMCKeychain* m0prv1pub2prv2pub1Gpub = [masterChain derivedKeychainWithPath:@"m/0'/1/2'/2/1000000000"];
     ...
     */
    {
        NSData* seed = DMCDataWithHexCString("fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542");
        
        DMCKeychain* masterChain = [[DMCKeychain alloc] initWithSeed:seed];

        //NSLog(@"identifier: %@ fingerprint: %@", masterChain.identifier, @(masterChain.fingerprint));
        NSAssert([masterChain.identifier isEqual:DMCDataFromHex(@"bd16bee53961a47d6ad888e29545434a89bdfe95")], @"");
        ...
        DMCKeychain* m0pub = [masterChain derivedKeychainAtIndex:0];
        ...
        DMCKeychain* m0pubFFprv = [m0pub derivedKeychainAtIndex:2147483647 hardened:YES];
        ...
        DMCKeychain* m0pubFFprv1 = [m0pubFFprv derivedKeychainAtIndex:1];
        ...
        DMCKeychain* m0pubFFprv1pubFEprv = [masterChain derivedKeychainWithPath:@"m/0/2147483647'/1/2147483646'"];
        ...
        DMCKeychain* m0pubFFprv1pubFEprv2 = [masterChain derivedKeychainWithPath:@"m/0/2147483647'/1/2147483646'/2"];
...
+ (void) testDeserializationWithKeychain:(DMCKeychain*)keychain {
    DMCKeychain* pubchain = [[DMCKeychain alloc] initWithExtendedKey:keychain.extendedPublicKey];
    DMCKeychain* prvchain = [[DMCKeychain alloc] initWithExtendedKey:keychain.extendedPrivateKey];
...
+ (void) testZeroPaddedPrivateKeys {
    DMCKeychain* keychain = [[DMCKeychain alloc] initWithSeed:[@"stress test" dataUsingEncoding:NSUTF8StringEncoding]];
    for (int i = 0; i < 2*256; i++)
    {
        //#puts i # i=70 yields a zero-prefixed private key.
        DMCKey* key = [keychain keyAtIndex:i hardened:YES];
