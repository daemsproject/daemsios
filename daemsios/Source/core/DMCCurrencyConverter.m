#import "DMCCurrencyConverter.h"
#import "DMCNumberFormatter.h"

@implementation DMCCurrencyConverter

- (id) init {
    if (self = [super init]) {
        _mode = DMCCurrencyConverterModeAverage;
    }
    return self;
}
...
- (DMCAmount) daemsCoinFromFiat:(NSDecimalNumber*)fiatAmount {
    switch (_mode) {
        case DMCCurrencyConverterModeAverage:
            if ([self isZero:_averageRate]) return 0;
            return DMCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_averageRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case DMCCurrencyConverterModeBuy:
            if ([self isZero:_buyRate]) return 0;
            return DMCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_buyRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case DMCCurrencyConverterModeSell:
            if ([self isZero:_sellRate]) return 0;
            return DMCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_sellRate] decimalNumberByMultiplyingByPowerOf10:8]);

            // TODO: add order book modes

        default:
            [[NSException exceptionWithName:@"DMCCurrencyConverter Not supported Mode" reason:@"This mode is not supported yet" userInfo:nil] raise];
            break;
    }
...
- (id) copyWithZone:(NSZone *)zone {
    return [[DMCCurrencyConverter alloc] initWithDictionary:self.dictionary];
}
