...
+ (DMCBigNumber*) curveOrder {
    static DMCBigNumber* order;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        order = [[DMCBigNumber alloc] initWithUnsignedBigEndian:DMCDataWithHexCString("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141")];
    });
    return order;
}
...
- (id) initEmpty {
    if (self = [super init]) {
        ...
        if (!_group) {
            NSLog(@"DMCCurvePoint: EC_GROUP_new_by_curve_name(NID_secp256k1) failed");
            goto finish;
        }
        
        _point = EC_POINT_new(_group);
        if (!_point) {
            NSLog(@"DMCCurvePoint: EC_POINT_new(_group) failed");
            goto finish;
        }
        
        _bnctx = BN_CTX_new();
        if (!_bnctx) {
            NSLog(@"DMCCurvePoint: BN_CTX_new() failed");
            goto finish;
        }
...
- (instancetype) add:(DMCCurvePoint*)otherPoint {
...
- (DMCBigNumber*) x {
    ...
    DMCBigNumber* result = [[DMCBigNumber alloc] initWithBIGNUM:bn];
    BN_CTX_end(_bnctx);
    return result;
}
...
- (id) copyWithZone:(NSZone *)zone {
    return [[DMCCurvePoint alloc] initWithEC_POINT:_point];
}

- (BOOL) isEqual:(DMCCurvePoint*)otherPoint {
...
- (NSString*) description {
    return [NSString stringWithFormat:@"<DMCCurvePoint:0x%p %@>", self, DMCHexFromData(self.data)];
}
