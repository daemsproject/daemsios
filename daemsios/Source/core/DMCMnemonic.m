...
@interface DMCMnemonic ()

@property(nonatomic, readwrite) DMCMnemonicWordListType wordListType;
...
@property(nonatomic, readwrite) DMCKeychain* keychain;

@end

static inline BOOL DMCMnemonicIsBitSet(uint8_t* buf, int bitIndex) {
...
static inline void DMCMnemonicSetBit(uint8_t* buf, int bitIndex) {
    int value = ((int) buf[bitIndex / 8]) & 0xFF;
    value = value | (1 << (7 - (bitIndex % 8)));
    buf[bitIndex / 8] = (uint8_t) value;
}

static inline void DMCMnemonicIntegerTo11Bits(uint8_t* buf, int bitIndex, int integer) {
    for (int i = 0; i < 11; i++) {
        if ((integer & 0x400) == 0x400) {
            DMCMnemonicSetBit(buf, bitIndex + i);
        }
        integer = integer << 1;
    }
}

static inline NSUInteger DMCMnemonicIntegerFrom11Bits(uint8_t* buf, int bitIndex) {
    NSUInteger value = 0;
    for (int i = 0; i < 11; i++) {
        if (DMCMnemonicIsBitSet(buf, bitIndex + i)) {
            value = (value << 1) | 0x01;
        } else {
            value = (value << 1);
        }
...
@implementation DMCMnemonic


- (id) initWithEntropy:(NSData*)entropy password:(NSString*)password wordListType:(DMCMnemonicWordListType)wordListType {
    if (!entropy) return nil;
    if (wordListType == DMCMnemonicWordListTypeUnknown) return nil;
...
- (id) initWithWords:(NSArray*)words password:(NSString*)password wordListType:(DMCMnemonicWordListType)wordListType {
    if (!words) return nil;
    ...
    if (self = [super init]) {
        ...
        if (wordListType != DMCMnemonicWordListTypeUnknown) {
...
- (id) initWithData:(NSData*)data {
    ...
    if (self = [super init]) {
        NSAssert(sizeof(DMCMnemonicWordListType) == 1, @"must be 1 byte long");

        if (data.length < sizeof(DMCMnemonicWordListType)) {
            // Not enough bytes.
            return nil;
        }
        ...
        _wordListType = ((DMCMnemonicWordListType*)data.bytes)[offset];

        offset += sizeof(DMCMnemonicWordListType);

        if (_wordListType != DMCMnemonicWordListTypeEnglish) {
            // Not supported list.
            return nil;
        }

        NSUInteger encodedEntropyLength = 0;
        _entropy = [DMCProtocolSerialization
        ...
        NSData* passwordData = [DMCProtocolSerialization
                                readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
        ...
        if (!passwordData) {
            ...
            NSLog(@"ERROR: DMCMnemonic cannot decode password (if missing, password should be empty string with 0x00 length prefix).");
            return nil;
        }
        ...
        if (!_password) {
            ...
            NSLog(@"ERROR: DMCMnemonic cannot convert password back to UTF8 string.");
            return nil;
        }

        // If we have some more data, try to read seed from there.
        if (data.length > offset) {
            _seed = [DMCProtocolSerialization
                     readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
                     readBytes:NULL];

            if (!_seed) {
                NSLog(@"ERROR: DMCMnemonic failed to read encoded seed from mnemonic payload. Will recompute it lazily when needed.");
            }
        }
    }
    return self;
...
- (DMCKeychain*) keychain
{
    if (!_keychain) {
        _keychain = [[DMCKeychain alloc] initWithSeed:self.seed];
    }
    return _keychain;
}
...
- (NSData*) dataWithSeed:(BOOL)withSeed {
    ...
    if (_wordListType == DMCMnemonicWordListTypeUnknown) return nil;

    [data appendBytes:&_wordListType length:sizeof(_wordListType)];

    if (!_entropy) return nil;

    [data appendData:[DMCProtocolSerialization dataForVarString:_entropy]];

    NSData* passwordData = [_password ?: @"" dataUsingEncoding:NSUTF8StringEncoding];

    [data appendData:[DMCProtocolSerialization dataForVarString:passwordData]];

    if (withSeed) {
        NSData* seed = self.seed;

        [data appendData:[DMCProtocolSerialization dataForVarString:seed]];
    }
...
- (BOOL) isEqual:(DMCMnemonic*)other {
...
- (NSArray*) wordsForEntropy:(NSData*)entropy wordListType:(DMCMnemonicWordListType)listType {
    if (!entropy) return nil;
    ...
    NSData* checksumHash = DMCSHA256(entropy);
    ...
    for (int i = 0; i < wordsCount; i++) {
        NSUInteger wordIndex = DMCMnemonicIntegerFrom11Bits((uint8_t*)buf.bytes, i * 11);
...
- (NSData*) entropyFromWords:(NSArray*)words wordListType:(DMCMnemonicWordListType)listType {
    if (listType == DMCMnemonicWordListTypeUnknown) return nil;
    if (!words) return nil;
    ...
    for (int i = 0; i < words.count; i++) {
        ...
        DMCMnemonicIntegerTo11Bits((uint8_t*)buf.mutableBytes, i * 11, (int)wordIndex);
    }
...
+ (NSArray*) wordListForType:(DMCMnemonicWordListType)type {
    if (type == DMCMnemonicWordListTypeEnglish) {
