...
typedef NS_ENUM(NSInteger, DMCProtobufType) {
    DMCProtobufTypeVarInt          = 0, // int32, int64, uint32, uint64, sint32, sint64, bool, enum
    DMCProtobufType64bit           = 1, // fixed64, sfixed64, double
    DMCProtobufTypeLengthDelimited = 2, // string, bytes, embedded messages, packed repeated fields
    DMCProtobufType32bit           = 5, // fixed32, sfixed32, float
};

@implementation DMCProtocolBuffers
...
+ (NSInteger) fieldAtOffset:(NSInteger *)offset int:(uint64_t *)i fixed32:(uint32_t *)fixed32 fixed64:(uint64_t *)fixed64 data:(NSData **)d fromData:(NSData*)src {
    ...
    switch (key & 0x07) {
        case DMCProtobufTypeVarInt: {
            varInt = [self varIntAtOffset:offset fromData:src];
            if (i) *i = varInt;
            break;
        }
        case DMCProtobufType64bit: {
            if (fixed64) {
...
+ (void) writeData:(NSData*)data withKey:(NSInteger)key toData:(NSMutableData*)dst {
    [self writeRawVarInt:(key << 3) + DMCProtobufTypeLengthDelimited toData:dst];
